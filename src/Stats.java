...
public class Stats { // Handles all the statistics (Times, Averages, personal bests etc)
    ...
    private static ArrayList<Integer> timesList = new ArrayList<>();
    ...
    private static File timeFile = new File(path + "/Times.csv");
    private static File userFile = new File(path + "/user.txt");

    public void loadTimes() { // loads the times from a csv file that is created on the users PC
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null, "File Times.csv could not be found", "ERROR",
    ...
    public static boolean checkIfNew() { // checks if the user is new by reading a file
        boolean isNew = true;
        BufferedReader br;
        try {
    ...
    private static void printNotNew() { // prints to the file so that the user no longer appears as new
        try {
            PrintWriter writer = new PrintWriter(new FileWriter(userFile));
            writer.print("*|*|*|User is not new*|*|*");
            writer.close();
        } catch (IOException e) {
